...
ret, frame = rec.read()
detected = detect_mask_tflite.detect(frame)
if detected:
    ...
    if detected:
        print("Alarm bajaidyo") #after making the code into a dependency for 'main.py', add return 1 here
...
if result:
    ...
    if isHome: # if owner is home
        ...
        if friendly.upper() == 'Y':
            ...
            print('Open gate') #return 0
        else:
            print('Gate is still closed') #return -1 because gate is closed yet alarm is not sounded
    else: #if owner is not home
        sendEmail.run()
        print('Alarm bajaidiyo') #return 1
